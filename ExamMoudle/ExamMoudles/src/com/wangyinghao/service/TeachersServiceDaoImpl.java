...
import com.wangyinghao.dao.impl.TeachersDaoImpl;
...
public class TeachersServiceDaoImpl implements TeachersServiceDao {
    ...
    public List<Teachers> selectteachertBynameOrtnumberOrtjob(Teachers teachers) {
        List<Teachers> teachersList= teachersDao.selectteachertBynameOrtnumberOrtjob(teachers);
        if (teachersList.size()==0||teachersList==null){
            return null;
        }else {
            return teachersList;
    ...
    public Teachers selectteacherByttid(Integer tid) {
        Teachers teachers =  teachersDao.selectteacherByttid(tid);
        if (teachers==null){
            return  null;
        }else {
            return teachers;
    ...
    public Teachers selectBytid( String tnumber) {
        ...
        //定义一个当前时间的字符串
        String NowData = "";
        List<Teachers> teachersList = teachersDao.selectteacherBytid(strNow+"___");
        if (teachersList.size()==0||teachersList==null){
    ...
    public void saveTeacherInfoAndUpdate(Teachers teachers) {
        ...
        teachersDao.saveTeacherInfoAndUpdate(teachers);
        return ;
